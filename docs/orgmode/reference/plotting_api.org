...
as NEB (=plot_landscape_surface= / =rgpycrumbs.surfaces=). Landfold
owns =F = -kT ln(rho/rhomax)=. Finite histogram cells become
=(s1, s2, F)= observations with grid slopes; =project_path= is
false because the plane is already the sketch-map.
...
Default =method= is =grad_imq= (eOn NEB plot TOML). Default
=surface_fit= thins the dense cloud to 300 points
(=SurfaceFitConfig(auto_thin=True, max_surface_points=300)=).
=fmax= clips finite =F= before the fit (JCTC 2013 panel uses 2).
