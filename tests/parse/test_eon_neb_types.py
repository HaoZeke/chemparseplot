...
def test_load_structures_and_calculate_additional_rmsd_returns_overlay_records(
    ...
    bundle = neb_mod.load_structures_and_calculate_additional_rmsd(
        con_file,
        [(add_file, "")],
        14.0,
        sp_file=sp_file,
    )
    assert isinstance(bundle, neb_mod.NebOverlayBundle)

    assert bundle.atoms_list == main_atoms
    assert len(bundle.additional_structures) == 1
    assert isinstance(bundle.additional_structures[0], neb_mod.NebOverlayStructure)
    assert bundle.additional_structures[0].atoms is add_atoms
    assert bundle.additional_structures[0].r == pytest.approx(3.5)
    assert bundle.additional_structures[0].p == pytest.approx(4.5)
    assert bundle.additional_structures[0].label == "extra"

    assert isinstance(bundle.saddle_point, neb_mod.NebOverlayStructure)
    assert bundle.saddle_point.atoms is sp_atoms
    assert bundle.saddle_point.r == pytest.approx(1.25)
    assert bundle.saddle_point.p == pytest.approx(2.5)
    assert bundle.saddle_point.label == "SP"
