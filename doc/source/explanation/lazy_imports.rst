...
The fallback produces the same dict format, with ``source``"legacy\ :sub:`interp`\"= instead of ``source``"opi"``. RMSD fields are =None`` because ``.interp`` files do not contain geometry data.
...
- `ORCA NEB Integration Design <orca_neb_design.rst>`_

- `NEB Parsing Design <neb_parsing_design.rst>`_

- `Architecture Overview <architecture.rst>`_
