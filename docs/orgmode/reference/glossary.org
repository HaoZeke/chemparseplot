...
MethodsX 2D landscape: a metric plane (RMSD =(r, p)=, rotated =(s, d)=,
or landfold =(s1, s2)=) plus potential energy and optional synthetic
gradients from =F_parallel=. Fitted by =plot.representation.plot_energy=.
Not occupancy invert.

*** landfold.fes.v1
Versioned landfold free-energy result: bin centres =x= / =y=, arrays
=free_energy= and =density= shaped =(ny, nx)=, plus =kt=.
=parse.landfold= loads the dict or the =landfold fes --csv= grid;
=plot.landfold.plot_fes= fits the NEB GP landscape
(=plot_landscape_surface=, default =grad_imq=).
