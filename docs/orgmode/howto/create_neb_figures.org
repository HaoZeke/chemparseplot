...
Requires eOn output with =write_movies=true= and a =climb= movie file. When
available, =climb.dat= remains the preferred table source; metadata-rich full
CON movies can also provide the per-iteration metrics directly.
