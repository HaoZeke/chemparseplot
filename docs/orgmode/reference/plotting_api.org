...
** Energy representation (MethodsX)

*** =chemparseplot.plot.representation.plot_energy(rep, ...)=

Potential energy on a metric 2D plane. =rep= is
=chemparseplot.energy.v1= / =EnergyRepresentation=: =(x, y, energy)=
plus optional =f_para= (synthetic path-tangent gradients, Savitzky-Golay
on the coordinates) or explicit =(grad_x, grad_y)=. Occupancy FES
results are rejected.

=frame="rmsd"= rotates =(r, p)= to =(s, d)= (=project_path= true).
=frame="landfold"= or ="progress"= keeps the given plane. Default
kernel =grad_imq=. Colorbar is =E=. Variance contours come from
=plot_landscape_surface=.

Build a path representation with
=chemparseplot.parse.representation.from_path_forces= or load
=# x y energy [f_para] [step]= via =load_energy_table=.
...
** Landfold occupancy FES

*** =chemparseplot.plot.landfold.plot_fes(fes_result, ...)=

Draw a landfold free-energy surface on the same GP landscape stack
as NEB (=plot_landscape_surface= / =rgpycrumbs.surfaces=). The fit
target is the observation cloud, not a clipped histogram of =F=.

- =on="density"= (default): unclipped =z = -kT ln(rho/rhomax)= on
  occupied bins
- =on="free-energy"=: landfold's =F= grid
- =cloud=(s1, s2, z[, gs1, gs2])=: frames, landmarks, or minima
  energies

=fmax= drops =z >= fmax= from the fit set; it does not clip =z=.
=project_path= is false: the plane is already the sketch-map.

Signature::

  plot_fes(
      fes_result, on="density", floor=0.0, cloud=None,
      xlabel=r"$s_1$", ylabel=r"$s_2$",
      clabel=None, fmax=None, points=None,
      method="grad_imq", surface_fit=None, n_inducing=None,
      show_pts=False, figsize=(5.6, 4.8), dpi=170,
  )

Default =method= is =grad_imq= (eOn NEB plot TOML). A value-only
cloud falls back to =rbf=. Default =surface_fit= thins to 300 points.
