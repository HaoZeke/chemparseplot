...
On a landfold sketch-map FES the same error is usually a missing IMQ
length scale (the plane spans tens of units, not a few angstrom of
RMSD). =plot_fes= supplies a span-based hint; =plot_landscape_surface=
keeps that hint when optimization returns a missing =ls=. Override with
=rbf_smooth= / ~rgpycrumbs landfold plot-fes --rbf-smooth~. See
[[file:create_landfold_fes.org][Draw a landfold FES]].
