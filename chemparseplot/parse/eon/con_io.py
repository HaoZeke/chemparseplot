...
metadata-native energies (``readcon>=0.14.7``) stay on one code path. ``readcon``
...
def _readcon():
    try:
        ...
        msg = (
            ...
            "pip install 'chemparseplot[neb]' (readcon>=0.14.7)"
        )
...
def frame_with_energy(frame: Any, energy: float) -> Any:
    ...
    Uses ``ConFrame.set_energy`` when available (readcon>=0.14.7) so metadata stays
